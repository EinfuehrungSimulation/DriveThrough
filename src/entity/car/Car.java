...
public class Car {
	...
	/**
	 * Create new EntityTypeAnimation
	 * @return
	 */
	...
	public Customer init(ModelAnimation owner){
		auto = new Customer(owner);
		return auto;
	}
//Creating the animation
	public Customer create(ModelAnimation owner, PositionExt pos){
		auto = new Customer(owner);
		auto.createAnimation(NAME, states.get(state).getName(), pos, true);
		return auto;
	}
	
	public Customer create(ModelAnimation owner){
		auto.createAnimation(NAME, states.get(state).getName(), true);
		return auto;
	}

	public Customer create(ModelAnimation owner, PositionExt pos, int state){
		this.state = state;
		auto = new Customer(owner);
		auto.createAnimation(NAME, states.get(state).getName(), pos, true);
		return auto;
